...
// Kyles Skill
Weapon.registerSkill("kyles", (player, { sp, velocity, notSelf, multiplier, team }, event) => {
	let maxDuration = 30
	...
	*/
	...
	if(player.isSneaking) {
      ...
      function kyleUlt() {
      	world.getDimension(player.dimension.id).getEntities({ maxDistance: 7, location: player.location, minDistance: 0, excludeNames: [...team], excludeTypes: ["minecraft:item","cz:indicator"] }).forEach(e => {
      	  let dmga = ((17*power)+e.getComponent("health").defaultValue/4)*(multiplier*2.8)
            if(dmga >= 2700) dmga = 2700
		    sp.bind(0.3)
            new Entity(e).addDamage(dmga, { cause: "entityAttack", damagingEntity: player }, { vel: velocity, hor: 0.5, ver: 0 })
      ...
      system.runTimeout(() => {
	    let kyle1 = Math.round((power * 43) + 28 + player.getComponent("health").defaultValue/4)
...
Weapon.registerSkill("silent", (player, lib, event) => {
	...
	if(player.isSneaking) {
	    ...
	    system.runTimeout(() => {
		  world.getDimension(player.dimension.id).getEntities({ location: player.location, maxDistance: 6, minDistance: 0, excludeNames: [...lib.team], excludeTypes: ["minecraft:item","cz:indicator"]}).forEach(e => {
...
Weapon.registerSkill("lectaze", (player, lib, event) => {
	...
	if(player.isSneaking) {// Skill 2
		...
		world.getDimension(player.dimension.id).getEntities({ maxDistance: 8, location: player.location, minDistance: 0, excludeNames: [...lib.team], excludeTypes: ["minecraft:item","cz:indicator"]}).forEach(e => {
...
Weapon.registerSkill("pandora", (player, lib, event) => {
	if(player.isSneaking == true && player.isOnGround == true) {// Skill 2
		...
		system.runTimeout(() => {
          world.getDimension(player.dimension.id).getEntities({ maxDistance: 3, location: player.location, minDistance: 0, excludeNames: [...lib.team], excludeTypes: ["minecraft:item","cz:indicator"] }).forEach(e => {
	  	...
	  	world.getDimension(player.dimension.id).getEntities({ maxDistance: 5, location: player.location, minDistance: 3.01, excludeNames: [...lib.team], excludeTypes: ["minecraft:item","cz:indicator"] }).forEach(e => {
...
Weapon.registerSkill("catlye", async (player, lib, event) => {
	...
	if(player.isSneaking == true) {// Skill 2
        ...
        system.runTimeout(() => {
	      world.getDimension(player.dimension.id).getEntities({ maxDistance: 6, location: player.location, minDistance: 0, excludeTypes: ["minecraft:item","cz:indicator"] }).forEach(i => {
...
Weapon.registerSkill("boltizer", (player, lib, event) => {
	...
	if(player.isSneaking == true) {// Skill 2
		...
		world.getDimension(player.dimension.id).getEntities({ maxDistance: 9, location: player.location, minDistance: 3, excludeNames: [...lib.team], excludeTypes: ["minecraft:item","cz:indicator"] }).forEach(e => {
			bolt(e)
